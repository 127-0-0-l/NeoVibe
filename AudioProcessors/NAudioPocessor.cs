...
namespace NeoVibe.AudioProcessors
{
    internal static class NAudioPocessor
    {
        ...
        internal static float[] GetFFT(int minFFTLength)
        {
            ...
            int FFTLength = 2048;//AudioUtils.GetFFTLength(minFFTLength) * 2;
            ...
            // do samplesRead = _fftReader.Read(readBuffer, 0, FFTLength);
            // while (_fftReader.Position < _audioFileReader.Position);
            //_fftReader.CurrentTime = _audioFileReader.CurrentTime;
            samplesRead = _fftReader.Read(readBuffer, 0, FFTLength);
            ...
            {
                complexBuffer[i].X = (float)(readBuffer[i]);
                complexBuffer[i].Y = 0;
            }
            ...
            for (int i = 0; i < result.Length; i++)
               result[i] = (float)(20 * Math.Log10(result[i] + 1e-6f)); // +epsilon to avoid log(0)

            float minDb = result.Min();
            float maxDb = result.Max();

            for (int i = 0; i < result.Length; i++)
            {
               float db = result[i];
               db = Math.Clamp(db, minDb, maxDb); // обрезаем шум
               result[i] = (result[i] - minDb) / (maxDb - minDb); // нормализуем в диапазон 0..1
            }
